...
# ─── LOAD DATA ──────────────────────────────────────────────────────────────
print("="*70)
print("LEDGERS OF THE SELF-EMPLOYED: Replication Script v4")
print("="*70)
print("\nLoading data...")
...
print("REPLICATION COMPLETE (v4)")


# ═══════════════════════════════════════════════════════════════════════════
# V4 ADDITIONS: 6-day sigma, anatomy, drift figure, gender robustness, Panel C
# ═══════════════════════════════════════════════════════════════════════════
print("V4: anatomy, 6-day sigma, drift, Panel C...")
from scipy import stats as sps
W_=13.64
own_=d5["phrs"]; fam_=d5["family_hours_weekly"].fillna(0)
int_=da["interest_paid"].values
pc_=d5["net_income_conventional"]; pa_=d5["net_income_adjusted"]
rows=[{"Line":"None (conventional cash)","Median charge":None,"Flips alone":None,"Cum loss":int((pc_<=0).sum())}]
cum=pc_.copy()
for nm,c in [("Owner labour",W_*own_),("Family labour",W_*fam_),
             ("Depreciation",d5["depreciation_weekly"]),("Informal interest",pd.Series(int_))]:
    alone=int((((pc_-c)<=0)&(pc_>0)).sum()); cum=cum-c
    rows.append({"Line":nm,"Median charge":round(float(c.median()),0),"Flips alone":alone,"Cum loss":int((cum<=0).sum())})
resid_=pc_-(W_*own_+W_*fam_+d5["depreciation_weekly"]+int_)-pa_
rows.append({"Line":"Other adjustments (net)","Median charge":round(float(resid_.median()),0),"Flips alone":0,"Cum loss":int(((cum-resid_)<=0).sum())})
pd.DataFrame(rows).to_csv(out("table13_anatomy.csv"),index=False)
print("  anatomy: cum ends at", rows[-1]["Cum loss"])

tc_=dt[~dt["is_imputed"]].copy()
tc_["flow"]=np.where(tc_["module"]=="M1A_VENTAS",tc_["amount_bs"],
    np.where(tc_["module"].isin(["M2_COMPRAS","M3_GASTOS_OP"]),-tc_["amount_bs"],0.0))
dd_=tc_.groupby(["firm_id","day_num"])["flow"].sum()
idx6=pd.MultiIndex.from_product([d5["firm_id"].unique(),range(1,7)],names=["firm_id","day_num"])
dd_=dd_.reindex(idx6,fill_value=0.0).reset_index()
dd_["resid"]=dd_["flow"]-dd_.groupby("day_num")["flow"].transform("mean")+dd_["flow"].mean()
dd_["dev"]=dd_["resid"]-dd_.groupby("firm_id")["resid"].transform("mean")
sig_d6=dd_.groupby("firm_id")["dev"].apply(lambda x: np.sqrt((x**2).sum()/5))
phi6=-0.202; k6=np.arange(1,6)
sig_w6=sig_d6*np.sqrt(6+2*np.sum((6-k6)*phi6**k6))
ffx=d5.set_index("firm_id").join(sig_w6.rename("sig"))
lm_=ffx["net_income_adjusted"]<=0
e_lv=sps.norm.sf(ffx.loc[lm_,"net_income_adjusted"].abs()/ffx.loc[lm_,"sig"]).sum()
e_vl=sps.norm.sf(ffx.loc[~lm_,"net_income_adjusted"].abs()/ffx.loc[~lm_,"sig"]).sum()
print(f"  6-day sigma median={sig_w6.median():.0f}, flips {e_lv:.0f}/{e_vl:.0f}, interval [{100*(214-e_lv)/500:.1f},{100*(214+e_vl)/500:.1f}]%")

rev_=tc_[tc_["module"]=="M1A_VENTAS"].groupby(["firm_id","day_num"])["amount_bs"].sum()
idx7=pd.MultiIndex.from_product([d5["firm_id"].unique(),range(1,8)],names=["firm_id","day_num"])
rev_=rev_.reindex(idx7,fill_value=0.0).reset_index()
dm_=rev_.groupby("day_num")["amount_bs"].agg(["mean","sem"])
fig,ax=plt.subplots(figsize=(7.2,4.0))
ax.bar(["Mon","Tue","Wed","Thu","Fri","Sat","Sun"],dm_["mean"],yerr=1.96*dm_["sem"],color="#00335f",alpha=.85,capsize=3)
ax.set_ylabel("Mean daily cash revenue (Bs)")
ax.set_title("Revenue by Day of the Diary Week (all diaries begin Monday)")
plt.tight_layout(); plt.savefig(out("figB13_weekday.pdf")); plt.savefig(out("figB13_weekday.png"),dpi=150); plt.close()

BV_=d5["cash_initial"]+d5["inventory_init"]+d5["receivables_init"]+d5["tool_value"]-d5["payables_init"]-d5["debt_amount"]
try:
    swf_=pd.read_csv(os.path.join(DATA_DIR,"diary_shadow_wages.csv"))
    fm_=d5.merge(swf_,on="folio",how="left")
    imp_h_=d5["imputed_labor_weekly"]/W_
    oth_=(pc_-pa_)-d5["imputed_labor_weekly"]-d5["depreciation_weekly"]
    pcrows=[]
    for col,lab in [("wstar_weekly_s3","Mincer all-salaried"),("wstar_weekly_s1","Mincer formal")]:
        wi_=np.maximum(fm_[col],546.0)
        pav=pc_-(wi_/40)*imp_h_-d5["depreciation_weekly"]-oth_
        vv=pav>0
        qq=((pav[vv&(BV_>0)]*52/0.18)/BV_[vv&(BV_>0)])
        pcrows.append({"Set":lab,"N":int(len(qq)),"Median Q":round(float(qq.median()),2),
                       "IQR":f"[{qq.quantile(.25):.1f},{qq.quantile(.75):.1f}]","Q>1 %":round(100*float((qq>1).mean()),0)})
    v0=pa_>0
    sm_=d5.loc[v0].groupby("sector")["net_income_adjusted"].transform("median")
    ps_=0.5*d5.loc[v0,"net_income_adjusted"]+0.5*sm_
    qs_=((ps_*52/0.18)/BV_[v0])[BV_[v0]>0]
    pcrows.append({"Set":"EB-shrunk (rel 0.5)","N":int(len(qs_)),"Median Q":round(float(qs_.median()),2),
                   "IQR":f"[{qs_.quantile(.25):.1f},{qs_.quantile(.75):.1f}]","Q>1 %":round(100*float((qs_>1).mean()),0)})
    pd.DataFrame(pcrows).to_csv(out("table6_panelC.csv"),index=False)
    print("  Panel C written")
except FileNotFoundError:
    print("  [shadow wages absent: Panel C skipped]")
print("REPLICATION COMPLETE (v4 additions)")
